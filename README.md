...
-	<ins>Role Based Policy Control</ins> – As different users will have different levels of view and modify access based on their responsibilities, the decision was made to implement a Role Based Policy Control.  This will allow the System Administrator to assign a role to a user that will limit their access to only those areas of the system that they have privileges to access *(GeeksforGeeks, n.d.-a)*.
